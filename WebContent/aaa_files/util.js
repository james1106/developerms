...
util.confirm_error = function(content) {
	util.confirm(content, '', '', {btn:['确定'], icon:2});
};
...
util.sms = {};
util.sms.check = function(callback) {
	var content = 
		'<div class="form-group clearfix">'+
			'<label class="col-sm-2 control-label">验证码</label>'+
			'<div class="col-sm-10 form-controls">'+
				'<div class="input-group">'+
					'<input type="text" class="form-control" id="captcha-text" placeholder="请输入验证码">'+
					'<div class="input-group-btn font-default">'+
						'<img id="captcha-code" onclick="this.src=\'./index.php?c=utility&a=code&v=\' + Math.random();" style="cursor:pointer; height: 34px;" src="./index.php?c=utility&a=code&v=1488354558" class="verification-img" alt="点击更换">'+
						'<a href="javascript:;" onclick="$(this).prev().trigger(\'click\')">看不清?</a>'+
					'</div>'+
				'</div>'+
			'</div>'+
		'</div>';
	...
	var $obj = util.dialog('发送短信验证', content, footer, {containerName : 'dialog-sms'});
	$obj.find('#captcha-code').trigger('click');
	$obj.find('#captcha-text').val('');
	$obj.modal('show');
	$obj.find('.js-sure').click(function() {
		var code = $obj.find('input').val();
		if (!code || code.length != 4) {
			util.confirm_error('验证码不正确');
			return false;
		};
		
		$obj.modal('hide');
		var url = "./index.php?c=utility&a=sms&do=check";
...
util.sms.send = function(mobile, callback) {
	var re = /^1\d{10}$/;
	if (!re.test(mobile)) {
		util.confirm_error('手机号不正确');
		return false;
	};
	var url = "./index.php?c=utility&a=sms&do=send";
...
util.sms.verifycode_check = function(mobile, verifycode, callback) {
	var url = "./index.php?c=utility&a=sms&do=verifycode_check";
	$.post(url, {mobile : mobile, verifycode : verifycode}, function(data) {
		if ($.isFunction(callback)) {
			callback(data);
		};
	}, 'json');
}
util.scrollFixedTop = function(ele){
	 var position = function() {
		 var top = ele.position().top;
		 var pos = ele.css("position");
		 $(window).scroll(function() {
			 var scrolls = $(this).scrollTop();
			 (scrolls > top) ? ele.css({position: "fixed", top: 0}) : ele.css({position: pos, top: top});
		 });
	};
 ...
 util.scrollFixedBottom = function(ele){
	 var position = function() {
		 var top = ele.position().top;
		 var pos = ele.css("position");
		 ...
		 var eleH = ele.height();
		 if(top > windowH){
			 ele.css({position: "fixed", top: windowH - eleH - 30});
		 }
		 $(window).scroll(function() {
			 var scrolls = $(this).scrollTop();
			 (scrolls > (top - windowH)) ? ele.css({position: pos, top: top}) : ele.css({position: "fixed", top: windowH - eleH - 30});
		 });
	 };
	 return $(this).each(function() {
		 position($(this));
 ...
 * @type String
 */
util.currencyFormat = function (num) {
