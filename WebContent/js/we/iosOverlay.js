...
var iosOverlay = function(params) {
	...
	// extend
	var type2fa = function(params){
		if (params['type'] && params['type'] in types){
			params['fa'] = types[params['type']];
		};
	};
	...
	var create = (function() {
		...
		if (params.icon) {
			...
			overlayDOM.innerHTML += '<i id="ios-fa" class="fa fa-5x fa-check '+params.fa+'"></i>';
		}
		if (doesTransitions) {
	...
	var update = function(params) {
		...
		if (params.fa){
			overlayDOM.getElementsByTagName('i')[0].setAttribute('class', 'fa fa-5x fa-check '+params.fa+'');
		}
		if (params.icon) {
			if (settings.spinner) {
